    ...
    A: AdviceProvider,
{
    ...
    fn handle_smt_delete(
        ...
        if node == Word::from(empty) {
            ...
            if leaf_key != key {
                ...
                // pair onto the advice stack and set key_not_set = ONE and is_leaf = ZERO
                ...
                let new_root = match self.find_lone_sibling(root, depth, &index)? {
                    Some((sibling, new_index)) => {
                        ...
                        // an finally we update the tree in the advice provider
    ...
    fn find_lone_sibling(
        ...
        for (depth, p_node) in (TieredSmt::TIER_SIZE..=depth).rev().zip(path.iter()) {
            ...
            // if the node is non-empty insert it into the map, but if a node for the same tier is
            // already in the map, return. We can do this because if a path has two non-empty nodes
            // at a given tier, then the node is not a lone sibling.
            let e_node = &empty[depth as usize];
            if p_node != e_node && non_empty_nodes.insert(tier, (depth, *p_node)).is_some() {
                return Ok(None);
            }
        }

        // take the deepest non-empty node and check if its subtree contains just a single leaf
        if let Some((_, (node_depth, node))) = non_empty_nodes.pop_last() {
